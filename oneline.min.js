...
BSON.checkKey=function checkKey(key,dollarsAndDotsOk){if(!key.length)return;if(!!~key.indexOf("\x00")){throw Error("key "+key+" must not contain null bytes");}
...
if(this.isNegative()){if(this.equals(Long.MIN_VALUE)){var radixLong=Long.fromNumber(radix);var div=this.div(radixLong);var rem=div.multiply(radixLong).subtract(this);return div.toString(radix)+rem.toInt().toString(radix);}else{return'-'+this.negate().toString(radix);}}
...
var res=Long.ZERO;var rem=this;while(rem.greaterThanOrEqual(other)){var approx=Math.max(1,Math.floor(rem.toNumber()/other.toNumber()));var log2=Math.ceil(Math.log(approx)/Math.LN2);var delta=(log2<=48)?1:Math.pow(2,log2-48);var approxRes=Long.fromNumber(approx);var approxRem=approxRes.multiply(other);while(approxRem.isNegative()||approxRem.greaterThan(rem)){approx-=delta;approxRes=Long.fromNumber(approx);approxRem=approxRes.multiply(other);}
...
exports.Symbol=Symbol;},'timestamp':function(module,exports,global,require,undefined){function Timestamp(low,high){if(!(this instanceof Timestamp))return new Timestamp(low,high);this._bsontype='Timestamp';this.low_=low|0;this.high_=high|0;};Timestamp.prototype.toInt=function(){return this.low_;};Timestamp.prototype.toNumber=function(){return this.high_*Timestamp.TWO_PWR_32_DBL_+
...
if(this.isZero()){return'0';}
if(this.isNegative()){if(this.equals(Timestamp.MIN_VALUE)){var radixTimestamp=Timestamp.fromNumber(radix);var div=this.div(radixTimestamp);var rem=div.multiply(radixTimestamp).subtract(this);return div.toString(radix)+rem.toInt().toString(radix);}else{return'-'+this.negate().toString(radix);}}
...
if(this.isNegative()){if(other.isNegative()){return this.negate().div(other.negate());}else{return this.negate().div(other).negate();}}else if(other.isNegative()){return this.div(other.negate()).negate();}
var res=Timestamp.ZERO;var rem=this;while(rem.greaterThanOrEqual(other)){var approx=Math.max(1,Math.floor(rem.toNumber()/other.toNumber()));var log2=Math.ceil(Math.log(approx)/Math.LN2);var delta=(log2<=48)?1:Math.pow(2,log2-48);var approxRes=Timestamp.fromNumber(approx);var approxRem=approxRes.multiply(other);while(approxRem.isNegative()||approxRem.greaterThan(rem)){approx-=delta;approxRes=Timestamp.fromNumber(approx);approxRem=approxRes.multiply(other);}
...
if(!needsMerge){Oneline.update=1;obj.state=0;Oneline.objects.push(obj);}};Oneline.once=function(event_type,message){var m={};var innerMessage={};var messageKeys=Object.keys(message);innerMessage['type']=event_type;innerMessage['data']={};for(var i in messageKeys){innerMessage['data'][messageKeys[i]]=message[messageKeys[i]];}
m.timestamp=new Date().getTime();m.connection_uuid=O.connection_uuid;m.module=O.module;m.uuid=O.uuid();m.packet={};m.packet.generic=innerMessage;Oneline.socket.send(Oneline.interop.stringify(m));};Oneline.alwaysConnect=function(){Oneline.alwaysConnectInterval=setInterval(function(){if(Oneline.socket.readyState===2||Oneline.socket.readyState===3){Oneline.connector.connect();}else{}},0);};Oneline.ready=function(callback){var readyInterval=setInterval(function(){if(Oneline.socket.readyState===1){callback();clearInterval(readyInterval);}else{Oneline.connector.connect();}},Oneline.readyIntervalFreq);};Oneline.clearCurrent=function(instance){for(var i in Oneline.objects){if(Oneline.objects[i]instanceof instance){delete Oneline.objects[i];}}};Oneline.getAfter=function(){return Oneline.after;};Oneline.setup=function(options,signature){Oneline.settings=options;Oneline.port=options.port||Oneline.port;Oneline.host=options.host||Oneline.host;Oneline.type=options.type==='bind'?'bind':'auto';Oneline.on=options.on||'click';Oneline.target=options.target;Oneline.signalStop=false;Oneline.connection_uuid=Oneline.connection_uuid||Oneline.uuid();if(typeof signature==='undefined'){Oneline.signature=Oneline.uuid();}
...
Oneline.settings.server=options.server='ws://'+Oneline.host+':'+Oneline.port+'/'+options.server;Oneline.module=options.server;if(OnelineTransport.WebSockets.detect()){Oneline.socket=OnelineTransport.WebSockets.Ctor(Oneline.settings.server);}else{if(OnelineTransport.XHR.detect()){Oneline.settings.xhrurl="http://"+Oneline.host+":"+((parseInt(Oneline.port)+1)).toString();Oneline.socket=OnelineTransport.XHR.Ctor(Oneline.settings.xhrurl,Oneline.settings.server);Oneline.loaded=true;}else{console.log("Neither XHR or WebSocket transport is available in this browser");}}
...
Oneline.target=document.getElementById(Oneline.target);Oneline.connector.connect();};Oneline.contextOptions={"geolocation":{"lat_field":{"default":"lat"},"lng_field":{"default":"lng"},"every":{"default":0.0},"limit":{"default":512},"range":{"default":40.00},"bidirectional":{"default":false}},"time":{"sfield":{"default":"stime"},"efield":{"default":"etime"}}};Oneline.fetchOptional=function(mod,option,value){if(typeof value==='undefined'){return Oneline.contextOptions[mod][option]['default'];}
...
return false;};Oneline.agent=function(options,ready){if(!ready){var obj=clone(Oneline.agent(options,1));obj.hashKey=Oneline.uuid();localStorage.set("key",obj.hashKey);}
return{delete:function(){if(typeof localStorage.get("key")!=="undefined"){delete localStorage['key'];}},run:function(){this.m={};this.m.hashKey=this.hashKey;this.state=1;}}};Oneline.nodes=function(options){};Oneline.stream=function(options){options=options||{};options.pipeline=options.pipeline||O.protoline;return O.pipeline({},options.pipeline,function(res){}).run();};Oneline.value=function(op,val){return{'op':op,'value':val};};Oneline.moment=function(start,end){return{'start':0000,'end':3600};};Oneline.echo=Oneline.echo=function(options,ready){Oneline.echo.options=options;if(!ready){var obj=clone(Oneline.echo(options,1));obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
...
return{run:function(m)
{this.m=m||{};this.m.time={};if(typeof Oneline.time.options.start!=='undefined'){this.m.time.start=Oneline.time.options.start;this.m.time.end=Oneline.time.options.end;this.m.time.on=Oneline.time.options.on;}else if(typeof Oneline.time.options.moment!=='undefined'){this.m.time.start=Oneline.time.options.moment.start;this.m.time.end=Oneline.time.options.moment.end;this.m.time.on=Oneline.time.options.moment.on;}
...
O.ooot=window['set'+O.runner](function(){if(c===O.objects.length&&O.objects.length>0){m_.packet=m;t=new Date().getTime();m_.timestamp=t;m_.module=O.module;m_.uuid=O.uuid();m_.connection_uuid=O.connection_uuid;if(t>Oneline.after){O.socket.send(O.interop.stringify(m_));}
window['clear'+O.runner](O.ooot);O.running=0;}else{window['clear'+O.runner](O.ooot);O.running=0;}},1);},O.freq);}};};Oneline.sound=function(options,ready){Oneline.sound.options=options;if(!ready){var obj=clone(Oneline.sound(options,1));obj.signature=Oneline.signature;obj.type="sound";obj.options=Oneline.sound.options;Oneline.joinOrNew(obj);}
return{run:function(m)
